...
const index = props => {
  ...
  const handleBlur = () => {
    if (errors !== "") seterrors(props.datos);
  };
  ...
  const handleSubmit = event => {
    ...
    if (
      ...
      if (
        /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(props.datos.email)
      ) {
        console.log(
          "La dirección de email " + props.datos.email + " es correcta."
        );
      } else {
        Swal.fire({
          icon: "error",
      ...
      Axios.post("/enviar", props.datos, props.typesCards)
        .then(response => {
          if (response.data.ok) {
            Swal.fire("Sended!", "We will contact you", "success");
            props.setdatos({
              name: "",
              phone: "",
              email: "",
        ...
        <div className="row-contact">
          ...
          <div className="c-contact">
            ...
            <div className="text-card">
              If you are interested or want to know more about our services, do
              not hesitate to contact us. Please leave your details and someone
              from EYSS Medical will get back to you very soon.
            </div>

            <form>
              <input
                onChange={e => {
                  props.setdatos({ ...props.datos, name: e.target.value });
                }}
                type="text"
                required
                onBlur={handleBlur}
              ...
              <input
                onBlur={handleBlur}
                onChange={e => {
                  props.setdatos({ ...props.datos, phone: e.target.value });
                }}
                type="number"
                required
                placeholder="Phone"
              ...
              <input
                onBlur={handleBlur}
                onChange={e => {
                  props.setdatos({ ...props.datos, email: e.target.value });
                }}
                type="email"
                required
                placeholder="Email"
              ...
              <textarea
                onBlur={handleBlur}
                onChange={e => {
                  props.setdatos({ ...props.datos, message: e.target.value });
                }}
                placeholder="Comment"
                required
                rows="6"
              ...
              <button onClick={handleSubmit} id="boton">
                Send
              </button>
            </form>
            ...
            <div>
              ...
              <div className="icont">
                ...
                <div className="i-tex">info@medic.eyss.io</div>
              </div>
